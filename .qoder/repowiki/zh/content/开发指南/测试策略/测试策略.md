...
**更新** 本版本反映了测试基础设施的大幅扩展，新增的单元测试现已覆盖所有主要功能区域，显著提升了代码质量保障能力。
...
- 单元测试：位于 src/__tests__/unit，覆盖运行时、工具链、上下文处理、权限与桥接等模块的大量测试用例，现已扩展至所有主要功能区域。
...
graph TB
subgraph "测试根目录"
E2E["e2e/*.spec.ts<br/>14个测试文件"]
INT["integration/*.test.ts<br/>3个测试文件"]
UNIT["unit/*.test.ts<br/>100+个测试文件"]
HELP["helpers.ts"]
...
### 单元测试扩展与覆盖策略
- 测试范围扩展
  - 新增单元测试现已覆盖所有主要功能区域，包括代理循环、任务执行、权限管理、上下文处理、桥接系统等核心模块。
- 测试用例组织
  - 按功能域划分测试文件，如 agent-loop-messages.test.ts、agent-task-runner.test.ts、permission-adapter.test.ts、context-assembler.test.ts 等。
- 覆盖率提升
  - 通过增加测试用例数量和改进测试策略，显著提升关键业务逻辑的测试覆盖率。

**章节来源**
- [agent-loop-messages.test.ts](file://src/__tests__/unit/agent-loop-messages.test.ts)
- [agent-task-runner.test.ts](file://src/__tests__/unit/agent-task-runner.test.ts)
...
## 结论
本测试策略以 Playwright 为核心，结合统一的辅助库与数据库隔离机制，构建了覆盖端到端、集成与单元测试的完整体系。通过明确的文件组织、断言规范与 CI/CD 流程，能够持续保障代码质量与用户体验。**最新的测试基础设施扩展显著增强了单元测试覆盖范围，现已涵盖所有主要功能区域，为代码质量提供了更全面的保障。** 建议在后续迭代中进一步完善覆盖率指标、扩展 API 测试与数据库测试场景，并将可视化测试与回归矩阵常态化纳入日常开发流程。
