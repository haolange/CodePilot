...
## 核心组件
- 消息输入框与提交按钮：负责文本输入、快捷键、粘贴、组合输入（IME）等事件处理，以及在流式响应期间切换为“停止”按钮。
...
## 结论
输入交互处理通过清晰的分层设计与完善的逻辑抽象，实现了从事件捕获到命令执行的完整闭环。键盘动作解析、徽章系统、弹出选择器与智能建议、安全过滤与性能优化共同构成了高可用、易扩展的输入体验。建议在后续迭代中持续完善测试覆盖与性能监控，确保在复杂场景下的稳定性与流畅度。
